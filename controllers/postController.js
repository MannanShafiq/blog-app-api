...
module.exports.createPost = (req, res) => {
    ...
    form.parse(req, async (error, fields, files) => {
        ...
        if(errors.length !== 0){
            ...
            const filepath = __dirname + `/../client/public/images/${files.image.newFilename}`;
                fs.copyFile(files.image.filepath, filepath, async (error) => {
                    if (!error) {
                        try {
          ...
          form.parse(req, (errors, fields, files) => {
            ...
            if(imageErrors.length !== 0){
                ...
                fs.copyFile(files.image.filepath, filepath, async (error) => {
                    if(!error){
                        try {
                            const response = await Post.findByIdAndUpdate(id, {Image: files.image.newFilename});
